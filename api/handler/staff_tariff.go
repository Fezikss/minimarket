...
func (h Handler) CreateStaffTariff(c *gin.Context) {
	...
	createdStaffTariff, err := h.storage.StaffTariff().GetById(context.Background(), models.PrimaryKey{ID: id})
	if err != nil {
...
func (h Handler) GetStaffTariff(c *gin.Context) {
	...
	staffTariff, err := h.storage.StaffTariff().GetById(context.Background(), models.PrimaryKey{ID: uid})
	if err != nil {
...
func (h Handler) GetStaffTariffList(c *gin.Context) {
	...
	response, err := h.storage.StaffTariff().GetList(context.Background(), models.GetListRequest{
		Page:   page,
		Limit:  limit,
		Search: search,
	})
...
func (h Handler) UpdateStaffTariff(c *gin.Context) {
	...
	sTariff.ID = uid
	if _, err := h.storage.StaffTariff().Update(context.Background(), sTariff); err != nil {
		handleResponse(c, "error while updating staff tariff", http.StatusInternalServerError, err.Error())
		return
	}

	updatedStaffTariff, err := h.storage.StaffTariff().GetById(context.Background(), models.PrimaryKey{ID: uid})
	if err != nil {
		handleResponse(c, "error while getting by ID", http.StatusInternalServerError, err.Error())
		return
	}
...
func (h Handler) DeleteStaffTariff(c *gin.Context) {
	uid := c.Param("id")
	id := models.PrimaryKey{ID: uid}

	if err := h.storage.StaffTariff().Delete(context.Background(), id); err != nil {
