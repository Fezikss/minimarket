...
func (b branchRepo) Create(branch models.CreateBranch) (string, error) {
	...
	if _, err := b.DB.Exec(`insert into branch (id, name, address, created_at) values($1, $2, $3, $4)`, id, branch.Name, branch.Address, branch.CreatedAt); err != nil {
...
func (b branchRepo) GetById(pk models.PrimaryKey) (models.Branch, error) {
	...
	if err := b.DB.QueryRow("select id, name, address, created_at from branch where id = $1", pk.ID).Scan(
		&branch.ID,
		&branch.Name,
		&branch.Address,
...
func (b branchRepo) Update(branch models.UpdateBranch) (string, error) {
	...
	if _, err := b.DB.Exec("update branch set name=$1, address=$2, update_at=$3 where id=$4", branch.Name, branch.Address, branch.UpdatedAt, branch.ID); err != nil {
		return "", err
	}
	if err := b.DB.QueryRow("select id, name, address, update_at, created_at from branch where id=$1", branch.ID).Scan(&branchs.ID, &branchs.Name, &branchs.Address, &branchs.UpdatedAt, &branchs.CreatedAt); err != nil {
