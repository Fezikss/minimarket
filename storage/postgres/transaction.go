...
func (t transactionRepo) GetById(ctx context.Context, id models.PrimaryKey) (models.Transaction, error) {
	trans := models.Transaction{}
	query := `select id, sale_id, staff_id, transaction_type, source_type, amount,
       						description, created_at, update_at
							from transaction where deleted_at is null and id = $1`
	if err := t.db.QueryRow(ctx, query, id.ID).Scan(
		&trans.ID,
		&trans.SaleID,
		&trans.StaffID,
		&trans.TransactionType,
...
func (t transactionRepo) GetList(ctx context.Context, request models.TransactionGetListRequest) (models.TransactionResponse, error) {
	...
	query = `select id, sale_id, staff_id, transaction_type, source_type, amount,
       						description, created_at, updated_at from transaction where deleted_at is null `
	...
	if err != nil {
		fmt.Println("error is while selecting all from transaction", err.Error())
		return models.TransactionResponse{}, err
	}
...
func (t transactionRepo) Update(ctx context.Context, transaction models.UpdateTransaction) (string, error) {
	query := `update transaction set sale_id = $1, staff_id = $2, transaction_type = $3, source_type = $4, amount = $5,
								description = $6, update_at = now() 
                    			where id = $7`
...
func (t transactionRepo) Delete(ctx context.Context, id models.PrimaryKey) error {
	query := `update transaction set deleted_at = now() where id = $1`
	if _, err := t.db.Exec(ctx, query, id.ID); err != nil {
