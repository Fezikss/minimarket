...
func (s *staffRepo) Create(ctx context.Context, staff models.CreateStaff) (string, error) {
	...
	if _, err := s.DB.Exec(ctx, `INSERT INTO staff
		(id, branch_id, tarif_id, staff_type, name, balance, age, birth_date, login, password)
...
func (s *staffRepo) GetById(ctx context.Context, id models.PrimaryKey) (models.Staff, error) {
	staff := models.Staff{}
...
func (s *staffRepo) GetList(ctx context.Context, request models.GetListRequest) (models.StaffsResponse, error) {
	...
	countQuery := `SELECT COUNT(*) FROM staff where deleted_at is null`
	if request.Search != "" {
		countQuery += fmt.Sprintf(` and staff_id= '%s'`, request.Search)
	}
...
`
	if request.Search != "" {
		query += fmt.Sprintf(` and staff_id= '%s'`, request.Search)
	}
...
func (s *staffRepo) Update(ctx context.Context, staff models.UpdateStaff) (string, error) {
	query := `UPDATE staff SET branch_id = $1, tariff_id = $2, staff_type = $3, 
...
func (s *staffRepo) Delete(ctx context.Context, id models.PrimaryKey) error {
	query := `UPDATE staff SET deleted_at = NOW() WHERE id = $1`
